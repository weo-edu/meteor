...
Meteor._Stream = function (url) {
  ...
  self.url = url; //Meteor._Stream._toSockjsUrl(url);
  self.socket = null;
...
_.extend(Meteor._Stream.prototype, {
  ...
  _launch_connection: function () {
    ...
    var opts = {
      resource: self.url.slice(1) + 'socket.io'
    };
    opts['force new connection'] = true;

    var _uniqueUri = io.util.uniqueUri;
    io.util.uniqueUri = function(uri){
      return _uniqueUri.call(this, uri) + '/' + opts.resource;
    };
    self.socket = io.connect('/', opts);
    io.util.uniqueUri = _uniqueUri;
